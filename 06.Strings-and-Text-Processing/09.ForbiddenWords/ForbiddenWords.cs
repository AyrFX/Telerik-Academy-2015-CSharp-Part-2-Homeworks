...
class ForbiddenWords
{
    static void Main()
    {
        ...
        string fbdWords = "PHP, CLR, Microsoft";
        string[] fbdWordsArray = fbdWords.Split(new[] {", "}, StringSplitOptions.RemoveEmptyEntries);
        for (int i = 0; i < fbdWordsArray.Length; i++)
        {
            text = text.Replace(fbdWordsArray[i], new String('*', fbdWordsArray[i].Length));            
        }
